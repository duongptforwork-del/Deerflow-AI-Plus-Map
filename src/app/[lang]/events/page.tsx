...
export default async function EventsPage({ params: { lang } }: { params: { lang: string } }) {
 ...
 <div key={event.id} className="group bg-white border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col h-full">
 {/* Date Header */}
 <div className="bg-black text-white p-6 border-b-4 border-black flex justify-between items-center transition-colors">
 ...
 <h2 className="text-3xl font-black tracking-tighter leading-none mb-6 uppercase transition-colors">
 <Link href={`/${lang}/events/${event.slug}`}>
 <span className="group-hover:text-[#ef4444] transition-colors">{event.title}</span>
 </Link>
 </h2>
 ...
 className="flex-1 text-center bg-white text-black py-4 border-4 border-black font-black uppercase text-xs shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] group"
 >
 <span className="group-hover:text-[#ef4444] transition-colors">Details</span>
 </Link>
 <Link 
 href={event.registration_link || '#'} 
 target="_blank"
 className="flex-1 text-center bg-[#ef4444] text-black py-4 border-4 border-black font-black uppercase text-xs shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
 >
 Tickets →
 </Link>
 </div>
 ...
 <p className="text-xl font-bold text-black/50 mt-4 uppercase">CHECK BACK SOON FOR THE NEXT WAVE OF INTELLIGENCE.</p>
 </div>
 )}
 </div>

 {/* Community Call to Action */}
 <div className="mt-24 bg-black p-12 border-4 border-black text-white text-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
 <h2 className="text-4xl font-black uppercase italic tracking-tighter mb-4">Hosting an Event?</h2>
 <p className="text-xl font-bold mb-8 text-white/70">Put your summit on the map and reach 500k+ AI professionals globally.</p>
