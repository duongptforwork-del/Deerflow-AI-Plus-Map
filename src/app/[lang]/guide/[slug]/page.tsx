...
export async function generateMetadata({ params: { lang, slug } }: { params: { lang: string; slug: string } }) {
 ...
 return {
 title: `${post.title} | AI Plus Map Guide`,
...
export default async function GuideDetailPage({ params: { lang, slug } }: { params: { lang: string; slug: string } }) {
 const supabase = createClient();
 ...
 return (
 <div className="min-h-screen bg-white text-black selection:bg-[#ef4444] selection:text-white">
 ...
 <h1 className="group-hover:text-[#ef4444] transition-colors text-2xl md:text-3xl lg:text-4xl font-black leading-tight mb-8 tracking-tight text-balance">
 {post.title}
 </h1>
 ...
 <p className="font-bold text-xs text-black/50 uppercase mt-1">Guide & Knowledge Division</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <Image fill src={rPost.featured_image || '/placeholder.png'} alt={rPost.title} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.src = '/placeholder.png'; }} />
 </div>
